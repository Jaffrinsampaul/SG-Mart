...
class _UserHomePageState extends State<UserHomePage> {
                              ...
                              children: [
                                ...
                                GestureDetector(
                                  onTap: ()async{
                                    ...
                                    var volume = 0;
                                    await data1.forEach((QueryDocumentSnapshot element) {
                                      print(element.get('name'));
                                      volume +=element.get('personalVolume');
                                    }
                                    );
                                    setState((){
                                      commssion =  ViewCommission(snapshot, data.get('personalVolume'),volume);
                                      groupvaolume = volume;
                                      FirebaseFirestore.instance
                                          .collection('Users')
                                          .doc(snapshot.data.id)
                                          .update({"commission": commssion, "groupVolume": volume});
                                    });
  ...
  int ViewCommission(AsyncSnapshot snapshot, pv, int gv) {
   ...
   int.parse((gv/100 * 7.5).toString()):
    ...
    pv >= 3000 && gv <= 12000?
    _buildErrorDialog(context, 'Eigther your personal volume is low or your group volume ');
  }
